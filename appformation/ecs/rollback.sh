...
# rollback to a taskdef revision via update-service
...
aws ecs update-service --cluster ${ecs_cluster_name} --service ${ecs_service_shortname} --task-definition ${task_definition_latest}:${revision}  --profile $profile
